...
from synthesizer.models.tacotron import SynthEmbedding
...
if __name__ == '__main__':
    ...
    parser.add_argument("-s", "--syn_model_fpath", type=Path,
                        default="saved_models/liepa2/synthesizer.pt",
    ...
    if isinstance(synthesizer._model.encoder.embedding, SynthEmbedding):
        mels = [np.load("/media/ratchet/hdd/Dataset/VoiceClone/Train/SV2TTS/synthesizer/mels/mel-7777-106366-0029_00.npy").T.astype(np.float32)]
    else:
    ...
    while True:
        try:
            ...
            if isinstance(synthesizer._model.encoder.embedding, SynthEmbedding):
                ...
                embed = torch.tensor([embed], device=synthesizer.device)

                mel_spectrogram = audio.melspectrogram(preprocessed_wav, synth_hparams).astype(np.float32)
                mel_spectrogram = torch.tensor([mel_spectrogram], device=synthesizer.device)
